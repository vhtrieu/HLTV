    ...
    public class HOCVIENsController : ApiController
    {
        ...
        public IHttpActionResult GetHOCVIEN(string SearchText)
        {
            var query = (from x in db.HOCVIENs
                         where x.HOV_FullName.ToLower().Contains(SearchText.ToLower())
                         || x.HOV_FirstName.ToLower().Contains(SearchText.ToLower())
                         || x.HOV_LastName.ToLower().Contains(SearchText.ToLower())
                         || x.HOV_Phone.ToLower().Contains(SearchText.ToLower())
                         || x.HOV_BirthDay.ToLower().Contains(SearchText.ToLower())
                         || x.HOV_Code.ToLower().Contains(SearchText.ToLower())
                         || SearchText==null
                         select new { 
                             HOV_ID = x.HOV_ID,
